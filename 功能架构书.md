...
### 1.2 前端实现 (`productDetail.jsp` - 新建)：
    ...
    *   **页面布局：**
        *   左侧：商品图片展示区 (主图，下方缩略图列表，点击切换)。
        *   右侧：商品信息区 (名称, 价格, 详细描述, 花语, “加入购物车”按钮, “立即购买”按钮)。
    *   **数据来源：** 由 `ProductDetailServlet` 通过 `request.setAttribute()` 传递 (`goods` 对象, `productDetailText` 字符串, `additionalImages` 列表)。
    *   **交互：**
        *   图片切换：使用 JavaScript (外部文件)。
        *   “加入购物车”按钮：GET 请求到 `/session?id=\${goods.id}`。
        *   “立即购买”按钮：GET 请求到 `/session?id=\${goods.id}&buyNow=true`。
...
### 1.3 后端实现 (`ProductDetailServlet.java` - 新建)：
    ...
    *   **`doGet` 方法逻辑：**
        1.  获取并校验商品 `id`。
        2.  调用服务层获取 `Goods` 对象。
        3.  通过 `goods.getProduct_id()` 调用服务层获取 `Product` 对象，从中提取 `detail` 文本。
        4.  根据主图URL和约定规则 (`[name]_[num].jpg`)，检查文件系统 (`web/images/`)，构建附加图片URL列表。
        5.  将 `goods`, `productDetailText`, `additionalImages` 存入 `request`。
        6.  转发到 `productDetail.jsp`。

### 1.4 数据库交互：

    *   查询 `goods` 表 (by `id`)。
    *   查询 `product` 表 (by `goods.product_id`)。

### 1.5 对现有文件的修改：

    *   `web/index.jsp`: 修改商品链接指向 `/productDetail?id=\${goods.id}`。
    *   `src/com/yang/controller/SessionServlet.java`: 在加入购物车逻辑后，检查 `buyNow` 参数，若为 `true` 则重定向到 `/shopCart`，否则重定向到 `/index`。
